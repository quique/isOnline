...
function check_site {
  ...
  if ! $QUIET; then echo -n "$p "; fi

  #if [ $response -eq 200 ] ; then
  if echo $response | grep $CANARY > /dev/null; then
    # website working
    if ! $QUIET; then echo -e "\e[32m[ok]\e[0m"; fi
    ...
    if [[ ! -f $WARNINGS/$filename ]]; then
      ...
      if ! $QUIET; then echo -e "\e[33m[down]\e[0m"; fi
      echo > $WARNINGS/$filename
    else
      # It keeps being down. Show an error.
      if ! $QUIET; then echo -e "\e[31m[DOWN]\e[0m"; fi
      if [[ ! -f $ERRORS/$filename ]]; then
        # 2nd time down.  Send email, but only if run by cron.
        if $QUIET; then
          while read e; do
